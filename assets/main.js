...
function createMovieCard(movie) {
    return `
        <div class="movie-card">
            <img src="${movie.poster}" alt="${movie.title}" onerror="this.src='https://via.placeholder.com/500x750.png?text=No+Poster'">
            <h3>${movie.title}</h3>
            <p>${movie.overview || 'No overview available.'}</p>
            <p><strong>Release:</strong> ${movie.release_date || 'Release date unknown'}</p>
            <p>⭐ Avg Rating: ${movie.average_rating}</p>
            <div class="stars" data-id="${movie.id}">
                ${createStarRating(movie.id)}
            </div>
        </div>
    `;
}
        ...
        $.get(`api/movie.php?id=${movieId}`)
            .done(function(data) {
                if (data && data.average_rating) {
                    movieCard.find('.avg-rating').text(`⭐ Avg Rating: ${data.average_rating}`);
                }
            });
    })
    .fail(function(error) {
...
function createMovieThumb(movie) {
    ...
    return `
        <div class="movie-thumb">
            <img src="${posterUrl}" alt="${movie.title}" onerror="this.src='https://via.placeholder.com/500x750.png?text=No+Poster'" />
